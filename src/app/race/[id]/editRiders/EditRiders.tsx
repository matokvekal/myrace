...
const EditRiders: React.FC<Props> = ({ raceUuid, categories }) => {
  const { riders, getRiders, addNewRider, updateRider, deleteRider, insertRiders } = useRiderStore();
  ...
  const cancelForm = () => { setAddingNew(false); setEditingId(null); };
  ...
  const saveForm = async () => {
    ...
    if (addingNew) {
      ...
      const existing = raceRiders.filter((r) => r.category === form.category);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (rows.length === 0) { setImportError("No valid rows found in file."); return; }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.toolbar}>
        ...
        <button className={styles.importBtn} onClick={() => fileRef.current?.click()}>📁 CSV</button>
        <input ref={fileRef} type="file" accept=".csv,.txt" hidden onChange={handleFileChange} />
      </div>
        ...
        <div className={styles.importBox}>
          <div className={styles.importTitle}>
            ...
            <span className={styles.importMeta}>
              {[...new Set(importPreview.map((r) => r.heat))].sort((a, b) => a - b).map((h) => (
                <span key={h}>  Wave {h}: {importPreview.filter((r) => r.heat === h).length}</span>
              ))}
            </span>
          </div>
          <div className={styles.importTable}>
            <div className={styles.importHeader}>
              <span>Bib</span><span>Name</span><span>Category</span><span>Wave</span>
            </div>
            ...
            {importPreview.length > 6 && (
              <div className={styles.importMore}>+{importPreview.length - 6} more riders…</div>
            )}
          </div>
          <div className={styles.importActions}>
            <button className={styles.cancelBtn} onClick={() => setImportPreview(null)}>Cancel</button>
      ...
      {(addingNew || editingId != null) && (
        <div className={styles.form}>
          <div className={styles.formTitle}>{addingNew ? "Add Rider" : "Edit Rider"}</div>
          <div className={styles.row2}>
            <input className={styles.input} placeholder="First name *" value={form.firstName} onChange={(e) => setF("firstName", e.target.value)} />
            <input className={styles.input} placeholder="Last name *" value={form.lastName} onChange={(e) => setF("lastName", e.target.value)} />
          </div>
          <div className={styles.row2}>
            <input className={styles.input} placeholder="Bib # *" type="number" value={form.bibNumber} onChange={(e) => setF("bibNumber", e.target.value)} />
            <input className={styles.input} placeholder="Chip #" value={form.chipNumber} onChange={(e) => setF("chipNumber", e.target.value)} />
          </div>
          <div className={styles.row2}>
            <select className={styles.input} value={form.category} onChange={(e) => setF("category", e.target.value)}>
              {categories.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
            </select>
            <select className={styles.input} value={form.heat} onChange={(e) => setF("heat", e.target.value)}>
              {waves.map((w) => <option key={w} value={w}>Wave {w}</option>)}
            </select>
          </div>
          <input className={styles.input} placeholder="Club / team" value={form.team} onChange={(e) => setF("team", e.target.value)} />
          <div className={styles.formActions}>
            <button className={styles.cancelBtn} onClick={cancelForm}>Cancel</button>
            <button className={styles.saveBtn} onClick={saveForm}>Save</button>
          </div>
        </div>
      )}
      ...
      <div className={styles.list}>
        {filtered.length === 0 && (
          <div className={styles.empty}>
            {search ? "No riders match." : "No riders yet — use + Add or import a CSV file."}
          </div>
        )}
        {filtered.map((rider) => (
          <div
          ...
          >
            ...
            <div className={styles.riderInfo}>
              <span className={styles.riderName}>{rider.lastName} {rider.firstName}</span>
              <span className={styles.riderMeta}>
                {rider.category} · Wave {rider.heat}
            ...
            <div className={styles.rowActions}>
              <button
                ...
                onClick={() => editingId === rider.id ? cancelForm() : startEdit(rider)}
              >
                ✎
              </button>
              <button
